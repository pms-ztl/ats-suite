"use client";
// app/(candidate-portal)/c/[slug]/jobs/page.tsx
// Tenant-scoped public job board. This is the per-tenant version of the rich
// /jobs board (the exact Candidate Portal.html port): CloudFront background
...
//
// Same faithful reproduction as the reference page (structure + scoped CSS are
// reproduced verbatim under a `.portalx` scope; @keyframes carry a `portal-`
// prefix; every inline script is reimplemented with React state/effects). The
// only differences from /jobs: the tenant slug is read via useParams, the
// tenant context (fetched name, or the slug) is shown in the nav/hero, and
// open roles are fetched tenant-scoped.
//
// The candidate-portal layout treats /c/{slug}/jobs as full-bleed, so this
// page ships its own complete chrome (nav + both footers).
//
// Open roles are best-effort fetched (tenant endpoint first, then the
// public board filtered by tenant, then the unscoped public board); on any
// failure we gracefully fall back to the design's three static example cards.
// Apply / role links go to /c/{slug}/jobs/{id}/apply.
...
function coerceRows(res: unknown): any[] {
  const rows: unknown = Array.isArray(res)
    ? res
    : ((res as any)?.data ?? (res as any)?.jobs ?? res ?? []);
...
// public board filtered by tenant slug, then the unscoped public board.
async function fetchJobs(slug: string): Promise<PortalJob[]> {
  const urls = [
    `${API_BASE}/c/${slug}/jobs`,
    `${API_BASE}/public/jobs?tenant=${encodeURIComponent(slug)}`,
    `${API_BASE}/public/jobs`,
  ];
  for (const url of urls) {
    try {
      const r = await fetch(url, { credentials: "include" });
      if (!r.ok) continue;
      const rows = mapJobs(coerceRows(await r.json()));
...
// Best-effort tenant display name (falls back to the slug at the call site).
async function fetchTenantName(slug: string): Promise<string | null> {
  try {
    const r = await fetch(`${API_BASE}/public/branding/${slug}`, { credentials: "include" });
    if (!r.ok) return null;
...
export default function TenantJobsPage() {
  // tenant slug from the route, with the tenant display name resolved best-effort
  const params = useParams<{ slug: string }>();
  const slug = typeof params?.slug === "string" ? params.slug : "";
  const [tenantName, setTenantName] = useState<string | null>(null);
  // What we show in the chrome: the fetched tenant name, else the slug, else a
  // neutral default so the hero never renders empty.
  const tenantLabel = tenantName ?? (slug ? slug : "your team");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!cancelled && name) setTenantName(name);
    })();
  ...
  return (
    <div className="portalx">
      ...
      <main>
        <div className="wrap">
          <nav>
            <a className="brand" href={`/c/${slug}/jobs`}><img src="/assets/logo-dark.png" alt={`${tenantLabel} careers`} style={{ height: 30, width: "auto", display: "block" }} /></a>
          ...
          <footer className="liquid-glass">
            <div className="f-top">
              <div>
                <div className="f-brand">
                  ...
                  <span className="nm">{tenantLabel}</span>
                </div>
                <p className="f-desc">The applicant-tracking platform behind {tenantLabel}'s hiring. Evidence-backed screening, human-in-the-loop decisions, and transparency for every candidate.</p>
              </div>
              <div className="f-links">
                ...
                <div className="f-col"><h4 className="up">Company</h4><a href="/welcome">Our story</a><a href="/welcome">Life here</a><a href="/welcome">Newsroom</a><a href="/welcome">Benefits</a></div>
            ...
            <div className="f-bottom">
              ...
              <div className="f-social">
                <span className="lbl up">Follow {tenantLabel}:</span>
        ...
        <div className="footer-wrapper">
          <div className="footer-left">
            ...
            <div className="footer-logo">
              <img src="/assets/logo-dark.png" alt={`${tenantLabel} careers`} style={{ height: 26, width: "auto", display: "block" }} />
            </div>
            ...
            <div className="footer-right-top">
              <div className="footer-nav-cols">
                ...
                <div className="footer-col">
                  ...
                  <a href={`/c/${slug}/jobs`}>Careers</a><a href="/welcome">About</a><a href="/transparency">Terms &amp; Conditions</a><a href="/transparency">Privacy Policy</a>
                </div>
              </div>
            </div>
            <div className="footer-bottom">
              <div className="footer-copyright">© 2026 {tenantLabel}. All rights reserved.<br /><span style={{ opacity: .9, display: "inline-flex", gap: "7px", flexWrap: "wrap", marginTop: "6px" }}><span>SOC 2 Type II</span> · <span>GDPR</span> · <span>EEOC-ready</span> · <a href="#" style={{ color: "#9ca3af", textDecoration: "underline" }}>System status</a></span></div>
