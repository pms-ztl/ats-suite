"use client";
// app/(dashboard)/admin/page.tsx - EXACT Claude Design "Aurora" admin command
// center (claude-design/dash-admin.jsx: AdminDash). The platform/tenant admin
// org-overview surface: CommandHero band with live toggle + hero stats, an 8->4
// KPI row, a charts row (pipeline funnel + diversity donut), a time-to-hire
// trend, an activity timeline, pending actions, and an advisory agent-activity
// panel. Gated to super-admin by app/(dashboard)/admin/layout.tsx (untouched).
//
// HONEST WIRING: the real data layer exposes three sources, so those carry real
// series with full loading/error/empty states and nothing is fabricated:
//   - getDashboardKpis  -> the CommandHero stats AND the KPI row (one source)
//   - getFunnel         -> the pipeline funnel
//   - getAdverseImpact  -> the diversity donut + the pending-actions flags
// Activity is a best-effort GET /platform/tenants (graceful: any failure or an
// empty payload falls back to the exact layout with an EmptyState). The
// time-to-hire trend has no wired series, so it renders the exact card shell
// with an EmptyState that links to the detail route rather than inventing data.
// The agent-activity panel is an advisory brand band (no per-agent counts).
import { useState } from "react";
import { CommandHero, KpiRow, SectionCard, Funnel, Donut, Timeline, PendingList, Reveal, Pill, Btn, type Kpi } from "@/components/aurora-kit";
...
// Best-effort raw GET for the activity feed; any failure resolves to [] so the
// section degrades gracefully to its EmptyState instead of an error wall.
async function rawList(path: string): Promise<any[]> {
  try {
    ...
    const a = Array.isArray(body?.data) ? body.data : Array.isArray(body) ? body : [];
    return a;
  } catch { return []; }
}

/* Friendly labels + canonical ordering for the funnel stages (matches the
   analytics port so the same pipeline reads identically across surfaces). */
...
// Calm brand->ai ramp so each funnel bar reads as one pipeline, not a rainbow.
...
// Advisory agent roster for the agent-activity band (brand copy, not metrics).
...
export default function AdminDash() {
  ...
  // ----- CommandHero stats, derived from the SAME real KPI source -----
  const heroStats = (kpis.data ?? []).map((k) => ({
    label: k.label, value: k.value, icon: k.icon, ai: k.ai, prefix: k.prefix, suffix: k.suffix, spark: k.spark,
  }));

  // ----- Funnel, ordered + labeled + colored from the real series -----
  ...
  const funnelStages = stages.map((s, i) => ({
    stage: STAGE_LABEL[s.stage] ?? s.stage, n: s.count, color: FUNNEL_COLORS[i % FUNNEL_COLORS.length],
  }));
  ...
  // ----- Diversity donut, normalised to a share of total selections -----
  const totalSel = (fairness.data ?? []).reduce((acc, m) => acc + (m.selectionRate || 0), 0) || 1;
  const donutData = (fairness.data ?? []).map((m, i) => ({
    g: m.group, v: Math.round((m.selectionRate / totalSel) * 100),
  ...
  // ----- Pending actions, derived from real fairness flags (no invented rows) -----
  const pending = flaggedGroups.map((g): { title: string; meta: string; ic: string; tone: "ok" | "warn" | "danger" } => ({
    ...
    meta: `Impact ratio ${g.impactRatio.toFixed(2)} · below 0.80 four-fifths threshold`,
    ic: "flag",
    tone: g.impactRatio < 0.6 ? "danger" : "warn",
  }));
  ...
  return (
    <div className="mx-auto w-full max-w-[1280px]">
      ...
      {/* KPI row, derived from the same real overview source */}
      ...
      {/* charts row: pipeline funnel + diversity donut */}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <Reveal i={11}>
            <SectionCard
              ...
              icon="listChecks"
              headRight={pending.length > 0 ? <Pill tone="var(--c-warn)" bg="var(--c-warn-tint)">{pending.length} need attention</Pill> : undefined}
            >
