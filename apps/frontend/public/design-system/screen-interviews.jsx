/* screen-interviews.jsx, Interviews list + detail (panelist feedback + AI interview-intelligence) */
const { useState: uSiv } = React;
const IV = window.UI;
const recTone = { STRONG_YES: "var(--ok)", YES: "var(--ok)", NEUTRAL: "var(--warn)", NO: "var(--danger)", STRONG_NO: "var(--danger)" };
function Dots({ n }) { return <span style={{ display: "inline-flex", gap: 3 }}>{Array.from({ length: 5 }).map((_, i) => <span key={i} style={{ width: 7, height: 7, borderRadius: 99, background: i < Math.round(n) ? "var(--brand)" : "var(--surface-3)" }} />)}</span>; }

function IVList({ onOpen }) {
  const [filter, setFilter] = uSiv("all");
  const rows = window.INTERVIEWS.filter(r => filter === "all" || r.status === filter);
  return (
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 16, flexWrap: "wrap", marginBottom: 16 }}>
          <div><h1 style={{ margin: 0, fontSize: "var(--fs-3xl)", fontWeight: 800, letterSpacing: "-0.03em" }}>Interviews</h1>
            <p style={{ margin: "5px 0 0", color: "var(--ink-2)", fontSize: "var(--fs-md)" }}>{window.INTERVIEWS.filter(r => r.status === "awaiting").length} awaiting feedback · {window.INTERVIEWS.filter(r => r.status === "scheduled").length} upcoming.</p></div>
          <IV.Btn variant="primary" icon="plus">Schedule interview</IV.Btn>
        </div>
        ...
        <div style={{ borderRadius: "var(--r-xl)", border: "1px solid var(--line)", background: "var(--surface)", overflow: "hidden", boxShadow: "var(--e1)" }}>
          {rows.map((r, i) => {
            const t = window.INTERVIEW_TYPES[r.type], st = window.IV_STATUS[r.status];
            return (
              <div key={r.id} onClick={() => onOpen(r.id)} style={{ display: "grid", gridTemplateColumns: "1.6fr 1.2fr 1fr 130px 120px", gap: 12, padding: "13px 18px", alignItems: "center", borderTop: i ? "1px solid var(--line)" : "none", cursor: "pointer", transition: "background var(--t-fast)" }}
                ...
                <div><div style={{ fontSize: 12.5, fontWeight: 600 }}>{r.round}</div><IV.Pill tone={t.tone} bg={"color-mix(in oklab," + t.tone + " 13%, transparent)"} style={{ fontSize: 10, marginTop: 2 }}>{t.label}</IV.Pill></div>
...
function IVDetail({ onBack }) {
  const d = window.IV_DETAIL;
  const t = window.INTERVIEW_TYPES[d.type];
  return (
    <div style={{ overflowY: "auto", height: "100%", padding: "18px 30px 50px" }}>
      <div style={{ maxWidth: 1080, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: 14, alignItems: "center", marginBottom: 20, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", gap: 9, alignItems: "center", flexWrap: "wrap" }}><h1 style={{ margin: 0, fontSize: "var(--fs-2xl)", fontWeight: 800, letterSpacing: "-0.02em" }}>{d.name}</h1><IV.Pill tone={t.tone} bg={"color-mix(in oklab," + t.tone + " 13%, transparent)"}>{t.label}</IV.Pill></div>
            <div style={{ fontSize: "var(--fs-sm)", color: "var(--ink-2)" }}>{d.round} · {d.role} · <span className="mono">{d.reqId}</span></div>
          </div>
          <IV.Btn variant="soft" icon="calendar">Reschedule</IV.Btn>
          <IV.Btn variant="primary" icon="check">Submit feedback</IV.Btn>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 18, alignItems: "start" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            ...
            <div style={{ borderRadius: "var(--r-xl)", border: "1px solid color-mix(in oklab, var(--ai) 22%, var(--line))", background: "var(--surface)", overflow: "hidden", boxShadow: "var(--e1)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "13px 18px", background: "linear-gradient(110deg, var(--ai-tint), transparent 65%)", borderBottom: "1px solid var(--line)" }}>
                <div style={{ display: "flex", gap: 9, alignItems: "center" }}><Icon name="sparkles" size={16} style={{ color: "var(--ai)" }} /><span style={{ fontWeight: 700, fontSize: "var(--fs-md)" }}>Interview intelligence</span><IV.Pill mono tone="var(--ai-ink)" bg="var(--ai-tint-2)">interview-intelligence</IV.Pill></div>
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}><IV.Pill tone="var(--ok)" bg="var(--ok-tint)" icon="check">{d.ai.rec}</IV.Pill><IV.Pill mono tone="var(--ai-ink)" bg="var(--ai-tint)">conf {d.ai.confidence.toFixed(2)}</IV.Pill></div>
              </div>
              <div style={{ padding: 18 }}>
                <p style={{ margin: "0 0 16px", fontSize: "var(--fs-sm)", color: "var(--ink-2)", lineHeight: 1.6 }}>{d.ai.summary}</p>
                <div style={{ ...IV.fStyles.label, marginBottom: 9 }}>Skill signals · with evidence</div>
                  ...
                  {d.ai.signals.map((s, i) => {
                    ...
                    return (
                      <div key={i} style={{ padding: "11px 13px", borderRadius: "var(--r)", border: "1px solid var(--line)", background: "var(--surface-2)" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}><span style={{ fontWeight: 600, fontSize: 12.5 }}>{s.skill}</span><IV.Pill tone={rt} bg={"color-mix(in oklab," + rt + " 13%, transparent)"} style={{ fontSize: 10 }}>{s.rating}</IV.Pill></div>
            ...
            <div style={{ borderRadius: "var(--r-xl)", border: "1px solid var(--line)", background: "var(--surface)", padding: 18, boxShadow: "var(--e1)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}><span style={{ fontWeight: 700, fontSize: "var(--fs-md)" }}>Panelist feedback</span><IV.Pill tone="var(--ink-2)">{d.panelists.filter(p => p.status === "submitted").length} / {d.panelists.length} submitted</IV.Pill></div>
                ...
                {d.panelists.map((p, i) => (
                  <div key={i} style={{ borderRadius: "var(--r-lg)", border: "1px solid var(--line)", padding: 14, background: p.status === "pending" ? "var(--surface-2)" : "var(--surface)" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      {p.status === "submitted" ? <div style={{ display: "flex", gap: 8, alignItems: "center" }}><span className="mono" style={{ fontSize: 13, fontWeight: 700 }}>{p.overall}</span><IV.Pill tone={recTone[p.rec]} bg={"color-mix(in oklab," + recTone[p.rec] + " 13%, transparent)"}>{p.rec.replace("_", " ")}</IV.Pill></div>
                        : <IV.Pill icon="clock" tone="var(--warn)" bg="var(--warn-tint)">pending</IV.Pill>}
                    </div>
                    {p.status === "submitted" && <>
                      ...
                      <p style={{ margin: 0, fontSize: 12, color: "var(--ink-2)", fontStyle: "italic", lineHeight: 1.45 }}>“{p.note}”</p>
                    </>}
                  </div>
                ))}
              </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ borderRadius: "var(--r-xl)", border: "1px solid var(--line)", background: "var(--surface)", padding: 18, boxShadow: "var(--e1)" }}>
              <div style={{ ...IV.fStyles.label, marginBottom: 4 }}>Details</div>
              {[["When", d.when], ["Duration", d.dur + " min"], ["Mode", d.mode], ["Round", d.round]].map(([k, v]) => <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "7px 0", borderTop: "1px solid var(--line)", fontSize: 12.5 }}><span style={{ color: "var(--ink-3)" }}>{k}</span><span style={{ fontWeight: 600 }}>{v}</span></div>)}
            ...
            <div style={{ borderRadius: "var(--r-xl)", border: "1px solid var(--line)", background: "var(--surface)", padding: 18, boxShadow: "var(--e1)" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                {["Walk through how you'd recover from a partial shard failure.", "How would you test the ledger's consistency guarantees?"].map((q, i) => <div key={i} style={{ fontSize: 12, color: "var(--ink-2)", padding: "8px 10px", borderRadius: "var(--r-sm)", background: "var(--surface-2)", lineHeight: 1.4 }}>{q}</div>)}
              </div>
...
function InterviewsScreen() {
  const [view, setView] = uSiv("list");
  return view === "detail" ? <IVDetail onBack={() => setView("list")} /> : <IVList onOpen={() => setView("detail")} />;
}
