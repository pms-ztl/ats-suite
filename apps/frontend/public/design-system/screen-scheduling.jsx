/* screen-scheduling.jsx, scheduling calendar with AI-proposed slots (advisory) */
const { useState: uSsc } = React;
const SC = window.UI;

function SchedulingScreen() {
  const s = window.SCHED;
  const [picked, setPicked] = uSsc(s.slots.findIndex(x => x.selected));
  const [booked, setBooked] = uSsc(false);
  const scoreTone = (v) => v >= 0.8 ? "var(--ok)" : v >= 0.6 ? "var(--warn)" : "var(--danger)";
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 380px", height: "100%", minHeight: 0 }}>
        ...
        <div style={{ borderRadius: "var(--r-xl)", border: "1px solid var(--line)", background: "var(--surface)", overflow: "hidden", boxShadow: "var(--e1)" }}>
          ...
          {s.hours.map((h, hi) => (
            <div key={h} style={{ display: "grid", gridTemplateColumns: "44px repeat(5, 1fr)", borderTop: hi ? "1px solid var(--line)" : "none", minHeight: 46 }}>
              ...
              {s.week.map(day => {
                const busyHere = Object.entries(s.busy).filter(([who, slots]) => slots.some(([d, a, b]) => d === day && hi >= a && hi < b));
                const proposed = s.slots.find(sl => sl.day.includes(day.split(" ")[0]) && parseInt(sl.time) === (hi < 3 ? 9 + hi : hi - 3) + (hi >= 3 && parseInt(sl.time) < 9 ? 0 : 0));
                // simpler: map proposed by index
                return (
      ...
      <aside style={{ borderLeft: "1px solid var(--line)", overflowY: "auto", padding: "22px 18px", background: "color-mix(in oklab, var(--surface) 50%, transparent)" }}>
        ...
        {booked ? (
          ...
          <SC.Btn variant="primary" icon="calendar" onClick={() => setBooked(true)} style={{ width: "100%", justifyContent: "center", marginTop: 14 }}>Book {s.slots[picked].time}, {s.slots[picked].day.split(" ").slice(1).join(" ")}</SC.Btn>
        )}
      </aside>
    </div>
  );
}
window.SchedulingScreen = SchedulingScreen;
