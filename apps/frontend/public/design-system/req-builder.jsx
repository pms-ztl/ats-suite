/* req-builder.jsx, interview-rounds config + application form-builder */
const { useState: uSb } = React;
const RB = window.UI;

/* ---------------- Interview rounds ---------------- */
function RoundsConfig() {
  const [rounds, setRounds] = uSb(window.ROUNDS.map(r => ({ ...r })));
  const move = (i, dir) => {
    const j = i + dir; if (j < 0 || j >= rounds.length) return;
    const next = [...rounds]; [next[i], next[j]] = [next[j], next[i]]; setRounds(next);
  };
  const toggleAuto = (id) => setRounds(rounds.map(r => r.id === id ? { ...r, auto: !r.auto } : r));
  ...
  return (
    <div style={{ maxWidth: 820 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        ...
        <RB.Btn variant="primary" icon="plus" onClick={add}>Add round</RB.Btn>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {rounds.map((r, i) => {
          const t = window.ROUND_TYPES[r.type];
          return (
            <div key={r.id} style={{ display: "flex", gap: 12, alignItems: "center", padding: "14px 16px", borderRadius: "var(--r-lg)", border: "1px solid var(--line)", background: "var(--surface)", boxShadow: "var(--e1)", animation: "rise .3s var(--ease-out)" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                  ...
                  <RB.Pill tone={t.tone} bg={"color-mix(in oklab," + t.tone + " 13%, transparent)"}>{t.label}</RB.Pill>
      ...
      <div style={{ marginTop: 14, padding: "11px 14px", borderRadius: "var(--r)", background: "var(--ai-tint)", border: "1px solid color-mix(in oklab, var(--ai) 20%, transparent)", display: "flex", gap: 9, alignItems: "center", fontSize: 12.5, color: "var(--ink-2)" }}>
        ...
        <RB.Btn variant="outlineAi" size="sm" icon="sparkles" style={{ marginLeft: "auto" }}>Generate kits</RB.Btn>
      </div>
    </div>
  );
}
...
function FormBuilder() {
  const [fields, setFields] = uSb(window.FORM_FIELDS.map(f => ({ ...f })));
  const [sel, setSel] = uSb(null);
  const add = (type, label) => setFields([...fields, { id: "f" + Date.now(), type, label: label + " field", required: false }]);
  const upd = (id, patch) => setFields(fields.map(f => f.id === id ? { ...f, ...patch } : f));
  const remove = (id) => setFields(fields.filter(f => f.id !== id || f.locked));
  const move = (i, dir) => { const j = i + dir; if (j < 0 || j >= fields.length) return; const n = [...fields]; [n[i], n[j]] = [n[j], n[i]]; setFields(n); };
  const typeIcon = { text: "type", textarea: "fileText", select: "chevD", checkbox: "check", file: "fileText", email: "dot" };

  return (
    <div style={{ display: "grid", gridTemplateColumns: "200px 1fr 1fr", gap: 16, alignItems: "start" }}>
      {/* palette */}
      <div>
        <div style={{ ...RB.fStyles.label, marginBottom: 10 }}>Add a field</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
          {window.FIELD_PALETTE.map(p => (
      ...
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div style={RB.fStyles.label}>Form structure · {fields.length} fields</div>
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {fields.map((f, i) => (
            <div key={f.id} onClick={() => setSel(f.id)} style={{ display: "flex", gap: 11, alignItems: "center", padding: "11px 13px", borderRadius: "var(--r)", border: "1px solid", borderColor: sel === f.id ? "var(--brand)" : "var(--line)", background: "var(--surface)", cursor: "pointer", boxShadow: sel === f.id ? "var(--ring)" : "var(--e1)" }}>
              ...
              <span style={{ width: 30, height: 30, borderRadius: 8, display: "grid", placeItems: "center", flexShrink: 0, background: "var(--surface-2)", color: "var(--ink-2)" }}><Icon name={typeIcon[f.type]} size={15} /></span>
              <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {!f.locked
                ...
                : <Icon name="shield" size={14} style={{ color: "var(--ink-3)" }} title="Locked default field" />}
            </div>
          ))}
        </div>
      </div>
      ...
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <span style={{ ...RB.fStyles.label, display: "inline-flex", gap: 6, alignItems: "center" }}><Icon name="eye" size={13} /> Candidate preview</span>
          <RB.Btn variant="primary" size="sm" icon="arrowUpRight">Publish</RB.Btn>
        </div>
        <div style={{ borderRadius: "var(--r-xl)", border: "1px solid var(--line)", background: "var(--surface)", padding: 18, boxShadow: "var(--e1)" }}>
          ...
          <div style={{ fontSize: 11.5, color: "var(--ink-3)", marginBottom: 16 }}>Northwind Talent · Payments</div>
            ...
            {fields.map(f => (
              <div key={f.id}>
                ...
                {f.type === "textarea" ? <div style={{ height: 56, borderRadius: "var(--r)", border: "1px solid var(--line-2)", background: "var(--surface-2)" }} />
                  : f.type === "select" ? <div style={{ height: 38, borderRadius: "var(--r)", border: "1px solid var(--line-2)", background: "var(--surface-2)", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 12px", fontSize: 12.5, color: "var(--ink-3)" }}>Select…<Icon name="chevD" size={14} /></div>
...
Object.assign(window, { RoundsConfig, FormBuilder });
