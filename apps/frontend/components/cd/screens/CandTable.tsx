"use client";
// components/screens/CandTable.tsx
// Dense high-volume triage table, ported pixel-exact from cand-table.jsx.
// Data via props. Selection is controlled by the parent (sel / setSel).
...
function StageBadge({ stage, stages }: { stage: string; stages: CandStage[] }) {
          ...
          return (
            <div key={c.id} onClick={() => onOpen(c.id)} style={{ display: "grid", gridTemplateColumns: cols, gap: 12, padding: pad, alignItems: "center", borderTop: i ? "1px solid var(--line)" : "none", cursor: "pointer", background: on ? "var(--brand-tint)" : "transparent", transition: "background var(--t-fast)" }}
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "center", minWidth: 0 }}>
                <span className="mono" style={{ width: 28, height: 28, borderRadius: 8, flexShrink: 0, display: "grid", placeItems: "center", fontWeight: 700, fontSize: 11, background: blind ? "var(--surface-3)" : "linear-gradient(135deg, var(--brand), var(--ai))", color: blind ? "var(--ink-3)" : "white" }}>{blind ? "•" : c.ini}</span>
              ...
              <div>
                {c.st !== "pending"
                  ? <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                      ...
                      <Icon name={stIcon(c.st)} size={13} stroke={2.3} style={{ color: stCol(c.st) }} />
                    </span>
                  : <Pill icon="clock" tone="var(--ink-3)" bg="var(--surface-2)">queued</Pill>}
              </div>
