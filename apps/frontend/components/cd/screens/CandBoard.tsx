"use client";
// components/screens/CandBoard.tsx
// Candidates kanban board (draggable, AI match badges, time-in-stage aging),
// ported pixel-exact from cand-board.jsx. Data via props.
...
function aging(days: number) {
  ...
  if (days >= 3) return { tone: "var(--warn)", bg: "var(--warn-tint)", label: days + "d", warn: false };
  return { tone: "var(--ink-3)", bg: "transparent", label: days === 0 ? "new" : days + "d", warn: false };
}

function CandCard({ c, onOpen, blind, onDragStart, dragging }: { c: Candidate; onOpen: (id: string) => void; blind: boolean; onDragStart: (e: React.DragEvent, id: string) => void; dragging: boolean }) {
  const age = aging(c.days);
  return (
    <div draggable onDragStart={(e) => onDragStart(e, c.id)} onClick={() => onOpen(c.id)}
      style={{ borderRadius: "var(--r-lg)", border: "1px solid var(--line)", background: "var(--surface)", padding: 13, boxShadow: "var(--e1)", cursor: "grab", opacity: dragging ? 0.4 : 1, transition: "box-shadow var(--t), transform var(--t-fast)", position: "relative" }}
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
        <span className="mono" style={{ width: 34, height: 34, borderRadius: "var(--r-sm)", flexShrink: 0, display: "grid", placeItems: "center", fontWeight: 700, fontSize: 12, background: blind ? "var(--surface-3)" : "linear-gradient(135deg, var(--brand), var(--ai))", color: blind ? "var(--ink-3)" : "white" }}>{blind ? "•" : c.ini}</span>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 11 }}>
        {c.st !== "pending" ? (
          <span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 8px", borderRadius: "var(--r-pill)", fontSize: 11, fontWeight: 700, color: "var(--ai-ink)", background: "var(--ai-tint)" }}>
            <Icon name="sparkles" size={11} /><span className="mono">{c.score}</span>
            <Icon name={stIcon(c.st)} size={11} style={{ color: stCol(c.st) }} />
          </span>
        ) : <Pill icon="clock" tone="var(--ink-3)" bg="var(--surface-2)">awaiting AI</Pill>}
...
export function CandBoard({ cands, stages, onMove, onOpen, blind }: { cands: Candidate[]; stages: CandStage[]; onMove: (id: string, stage: string) => void; onOpen: (id: string) => void; blind: boolean }) {
  const [dragId, setDragId] = useState<string | null>(null);
  const [overCol, setOverCol] = useState<string | null>(null);
  const onDragStart = (e: React.DragEvent, id: string) => { setDragId(id); e.dataTransfer.effectAllowed = "move"; };
  const onDrop = (stage: string) => { if (dragId) onMove(dragId, stage); setDragId(null); setOverCol(null); };
  return (
      ...
      {stages.map((stage) => {
        ...
        return (
          <div key={stage.id} onDragOver={(e) => { e.preventDefault(); setOverCol(stage.id); }} onDragLeave={() => setOverCol((o) => (o === stage.id ? null : o))} onDrop={() => onDrop(stage.id)}
            style={{ width: 268, flexShrink: 0, display: "flex", flexDirection: "column", minHeight: 0, borderRadius: "var(--r-xl)", background: over ? "var(--brand-tint)" : "var(--surface-2)", border: "1px solid", borderColor: over ? "var(--brand)" : "var(--line)", transition: "background var(--t), border-color var(--t)" }}>
