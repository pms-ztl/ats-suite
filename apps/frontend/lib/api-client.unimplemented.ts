// lib/api-client.unimplemented.ts
// ─────────────────────────────────────────────────────────────────────────────
// PARKED / UNWIRED CLIENT METHODS — NOT USED ANYWHERE IN THE APP.
//
// Nothing imports this file. It exists only to keep a labelled record of client
// methods that were removed from ./api-client.ts because they are not wired to any
// UI (the app's data layer goes UI → lib/api.ts → api-client.ts, which now exposes
// only the 5 namespaces the UI actually calls: platform, candidates, interviews,
// screening, decisions).
//
// ⚠️ MOST OF THESE HAVE NO BACKEND. The api-gateway returns 404 (ROUTE_NOT_FOUND)
// for the bias/*, compliance/*, security/*, ai/* and analytics/* (dashboard/funnel/
// export) paths — they advertise EU-AI-Act / NYC-LL144 / prompt-firewall style
// tooling that DOES NOT EXIST. Do not demo or promise anything from them. The only
// real compliance surface is GET /screening/audit/:requisitionId + /analytics/diversity.
//
// A few paths here (requisitions/*, sourcing/*, some analytics) DO have live routes,
// but no UI calls them through this client, so they are parked here too rather than
// masquerading as part of the wired client.
//
// This file is self-contained (its own copy of the request helpers) so it never
// couples the live client to dead code. If you wire any of these up, move the method
// back into api-client.ts and add its caller to lib/api.ts.
// ─────────────────────────────────────────────────────────────────────────────
