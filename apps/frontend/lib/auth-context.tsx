...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    apiFetch("/auth/me", { headers: { Authorization: `Bearer ${stored}` } })
      .then(async res => {
        if (res.ok) {
          ...
          const u = data.data;
          setAuth({ id: u.id, email: u.email, name: u.name ?? `${u.firstName} ${u.lastName}`, role: u.role, tenantId: u.tenantId, tenant: u.tenant }, stored);
        } else {
          clearAuth();
        }
      })
  ...
  const login = useCallback(async (email: string, password: string) => {
    ...
    setAuth({ id: user.id, email: user.email, name: user.name, role: user.role, tenantId: user.tenantId, tenant: user.tenant }, token);
