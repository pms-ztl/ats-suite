...
// Trusted system client (migrations + NATS subscribers that open a case with an
// explicit tenant filter). Not subject to RLS. Never use on user request paths.
export const prismaAdmin: PrismaClient =
  globalFor.prismaAdmin ?? new PrismaClient({ log: logLevels });

// Request-path client. With ONBOARDING_APP_DATABASE_URL pointing at the non-
// superuser ats_app role, RLS policies apply and the extension stamps the per-
// request tenant onto the DB session. Unset → falls back to admin (RLS inert).
const appUrl = process.env["ONBOARDING_APP_DATABASE_URL"];
