...
// Trusted system client (migrations + any cross-tenant maintenance). Superuser →
// not subject to RLS. Never use for user request paths.
export const prismaAdmin: PrismaClient =
  globalFor.prismaAdmin ?? new PrismaClient({ log: logLevels });

// Request-path client. With INTERVIEW_APP_DATABASE_URL pointing at the non-
// superuser ats_app role, RLS policies apply and the extension stamps the
// per-request tenant onto the DB session. Unset → falls back to admin (RLS inert).
const appUrl = process.env["INTERVIEW_APP_DATABASE_URL"];
