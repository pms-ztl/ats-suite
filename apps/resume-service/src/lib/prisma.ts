...
// Trusted system client (migrations + the background resume-parse worker and
// parse pipeline, which run outside any HTTP request and scope by tenantId
// explicitly). Superuser → not subject to RLS. Never use for user request paths.
export const prismaAdmin: PrismaClient =
  globalFor.prismaAdmin ?? new PrismaClient({ log: logLevels });

// Request-path client. With RESUME_APP_DATABASE_URL pointing at the non-
...
const appUrl = process.env["RESUME_APP_DATABASE_URL"];
